		...
		if qcVxNet.Router != nil {
			...
			if vxnetItem.TunnelType == constants.TunnelTypeVlan {
				if customReservedIPCount > 0 {
					...
					maxReservedCount := int64(count) * (1 - 1/reservedVIPCountForVlan)
					if maxReservedCount < customReservedIPCount {
						return nil, fmt.Errorf("invalid customReservedIPCount %d config in vxnetpool spec", customReservedIPCount)
					}
