	...
	for _, qcVxNet := range output.VxNetSet {
		...
		if qcVxNet.Router != nil {
			...
			if vxnetItem.TunnelType == constants.TunnelTypeVlan {
				if customReservedIPCount > 0 {
					count := IPRangeCount(*qcVxNet.Router.DYNIPStart, *qcVxNet.Router.DYNIPEnd)
					maxReservedCount := int64(count) - int64(count)/reservedVIPCountForVlan
					if maxReservedCount < customReservedIPCount {
						return nil, fmt.Errorf("invalid customReservedIPCount %d config in vxnetpool spec, should in range 0~%d", customReservedIPCount, maxReservedCount)
					}
				...
				vxnetItem.IPEnd = getIPEndAfterReserved(*qcVxNet.Router.DYNIPEnd, reservedVIPCount, 0)
			}
		} else {
