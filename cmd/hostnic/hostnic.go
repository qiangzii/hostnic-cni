...
func cmdAddVeth(conf constants.NetConf, hostIfName, contIfName string, msg *rpc.IPAMMessage, result *current.Result, netns ns.NetNS) error {
	...
	if link != nil {
		logrus.Infof("cmdAddVeth LinkByName found intf %s, link type=%s, link attr=%s,err=%v; going to delete this intf", hostIfName, link.Type(), spew.Sdump(link.Attrs()), err)
		veth := &netlink.Veth{
		...
		if err != nil {
			logrus.Errorf("delete intf %s error: %v", hostIfName, err)
			return err
		}
		logrus.Infof("delete intf %s success!", hostIfName)
	}
